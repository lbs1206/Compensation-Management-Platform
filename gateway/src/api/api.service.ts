...
export class ApiService {
    ...
    // Auth 서비스로 요청 전달
    async forwardToAuth(method: string, path: string, body?: any, headers?: any, query?: any): Promise<any> {
        const auth_service_url = this.configService.get<string>('AUTH_SERVICE_URL') || 'http://localhost:3010';
        const url = `${auth_service_url}/${path}`;
        return this.forwardRequest(method, url, body, headers, query);
    }

    // Event 서비스로 요청 전달
    async forwardToEvent(method: string, path: string, body?: any, headers?: any, query?: any): Promise<any> {
        const event_service_url = this.configService.get<string>('EVENT_SERVICE_URL') || 'http://localhost:3020';
